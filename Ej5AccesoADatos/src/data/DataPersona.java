...
public class DataPersona {
	...
	public void add(Persona p) { 
		...
		try {
            ...
            if(keyResultSet!=null && keyResultSet.next()){
                p.setId(keyResultSet.getInt(1)); //por ser autoincremental!
            }
	...
	public LinkedList<Persona> getByApellido(Persona per) {
		...
		try {
			stmt= DbConnector.getInstancia().getConn().prepareStatement("select id,nombre,apellido,tipo_doc,nro_doc,email,tel,habilitado from persona where "
					+ "apellido=?");
	...
	public Persona editPersona (Persona p) {
		...
		try {
			stmt=DbConnector.getInstancia().getConn().
					prepareStatement(
							"UPDATE `java`.`persona` SET `nombre` = ?, `apellido` = ?, `email` = ?, `password` = ?, `tel` = ?, `habilitado` = ? WHERE (`id` = ?)", PreparedStatement.RETURN_GENERATED_KEYS);
			stmt.setString(1, p.getNombre());
			stmt.setString(2, p.getApellido());
	...
	public Persona deletePersona(Persona p) {
		...
		try {
			stmt=DbConnector.getInstancia().getConn().
					prepareStatement(
							"delete from persona where persona.id=? ", PreparedStatement.RETURN_GENERATED_KEYS);
